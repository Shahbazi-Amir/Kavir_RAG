...
def ingest_file(path: Union[str, Path], out_dir: Path = CHUNKS_DIR, ocr: bool = False) -> None:
    ...
    text = load_any_bytes(data, ext=ext, ocr=ocr)
    ...
    for idx, chunk in enumerate(raw_chunks):
        ...
        text_chunks.append(
            TextChunk(
                ...
                end_char=end_char,
                overlap=overlap,
                source={"path": str(fpath), "page_start": None, "page_end": None, "mime": f"file/{ext}"},
    ...
    print(f"Ingested {fpath} ({ext}) → {len(text_chunks)} chunks")
